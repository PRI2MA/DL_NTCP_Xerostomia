...
import math
import torch
from .layers import conv3d_padding_same, depthwise_separable_conv, Output, pooling_conv


class conv_block(torch.nn.Module):
    def __init__(self, in_channels, filters, kernel_size, strides, pad_value, lrelu_alpha, use_activation,
                 use_dws, use_bias=False):
        ...
        self.conv1_dws = depthwise_separable_conv(in_channels=in_channels, out_channels=filters, stride=strides,
                                                 kernel_size=kernel_size, bias=use_bias)
...
class DCNN_DWS_LReLU(torch.nn.Module):
    """
    Deep CNN with depthwise separable conv
    """
    ...
    def __init__(self, n_input_channels, depth, height, width, n_features, num_classes, filters, kernel_sizes, strides,
        ...
        super(DCNN_DWS_LReLU, self).__init__()
        ...
        for i in range(len(in_channels)):
            ...
            self.conv_blocks.add_module('conv_block%s' % i,
                                        conv_block(in_channels=in_channels[i], filters=filters[i],
                                                   ...
                                                   use_activation=use_activation, use_bias=use_bias, use_dws=use_dws))
