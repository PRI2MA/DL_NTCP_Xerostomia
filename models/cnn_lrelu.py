...
class conv_block(torch.nn.Module):
    def __init__(self, in_channels, filters, kernel_size, strides, pad_value, lrelu_alpha, use_bias=False):
...
class CNN_LReLU(torch.nn.Module):
    """
    Traditional CNN
    """
        ...
        for i in range(len(in_channels)):
            self.conv_blocks.add_module('conv_block%s' % i,
                                        conv_block(in_channels=in_channels[i], filters=filters[i],
                                                   ...
                                                   pad_value=pad_value, lrelu_alpha=lrelu_alpha, use_bias=use_bias))
